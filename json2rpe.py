...
def convert(json_path, rpe_path):
    ...
    # 写入info.txt
    info = "#\nName: {}\nPath: {}\nSong: {}\nPicture: {}\nChart: {}\nLevel: {}\nComposer: {}\nCharter: {}".format(
        rpe["META"]["name"], rpe["META"]["id"], rpe["META"]["song"], rpe["META"]["background"], b, rpe["META"]["level"],
        rpe["META"]["composer"], rpe["META"]["charter"])
    ...
    for judgeline in file_json["judgeLineList"]:
        rpe["judgeLineList"].append({"Group": 0, "Name": "Untitled", "Texture": "line.png", "eventLayers": [
            {"alphaEvents": [], "moveXEvents": [], "moveYEvents": [], "rotateEvents": [], "speedEvents": []}]})
        # 判定线不透明度
                    ...
                    rpe["judgeLineList"][count]["notes"].append(
                        {
                            "above": 2,
                            "alpha": 255,
                            "endTime": [math.floor((i["time"] + i["holdTime"]) / 32), int(i["time"] + i["holdTime"]) % 32,
                                        32],
                            "isFake": 0,
                            "positionX": i["positionX"] * (675.0 / 9),
                            "size": 1.0,
